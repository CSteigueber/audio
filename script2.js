...
var nsst=sst[years.indexOf(year)]; // setting nsst to the value cooresponding to the beginning year
...
document.getElementById("poprun").addEventListener("click",function (){
    ...
    (function manualLoop() {
        setTimeout(function() {
            ...
            let i=years.indexOf(Math.floor(year));
            if ((i!=-1)&&(year<2014)){
                let lastYear=years[i];
                let nextYear=years[i+1];
                let lastsst=sst[i];
                let nextsst=sst[i+1];
                delta= (nextsst-lastsst)/10;   
            }
            nsst+=delta;
            document.getElementById("pop").innerHTML=`SST: ${(nsst).toFixed(2)} °C from normal`;
            document.getElementById("year").innerHTML=`Year: ${year.toFixed(0)}`;
            frequency=numToFreq(nsst);
            oscillatorNode.frequency.value = frequency;
